...
public class BracketeeringEngine : BaseJackboxEngine<BracketeeringClient>
{
    ...
    public BracketeeringEngine(ICompletionService completionService, ILogger logger, BracketeeringClient client, ManagedConfigFile configFile, int instance, uint coinFlip)
        : base(completionService, logger, client, configFile, instance)
    {
        if (configFile.Bracketeering.ChatPersonalityChance > RandGen.NextDouble())
    ...
    private async void FormEntry(BracketeeringPlayer self)
    {
        ...
        LogInfo($"Asking GPT for lie in response to \"{prompt}\"");
    ...
    private async Task<string> ProvideResponse(string entryPrompt, int maxLength)
    {
        ...
        {
            ChatSystemMessage = "You are a player in a game called Bracketeering, in which players attempt to write silly responses to prompts. Please respond to the prompt with just your short answer, do not repeat the prompt.",
...
Response:",
    ...
    {
        if (RandGen.Next() > Config.Model.VotingStrayChance)
        ...
        var prompt = new TextInput
        {
            ChatSystemMessage = $"You are a player in a game called Bracketeering, in which players attempt to come up with silly responses to prompts. You will be given two responses for the prompt \"{choicePrompt}\", please respond with the number corresponding to the option that you think is the best.",
